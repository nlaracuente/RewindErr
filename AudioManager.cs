...
public class AudioManager : Singleton<AudioManager>
{
    ...
    private void Update()
    {
        ...
        // Remove all nulls
        m_soundFxSources = m_soundFxSources.Where(s => s != null).ToList();
    }
    ...
    /// <summary>
    /// Chooses a random clip from the clips clips collectiong
    /// </summary>
    /// <param name="infos"></param>
    /// <returns></returns>
    public AudioClipInfo GetRandomAudioClipInfo(List<AudioClipInfo> infos)
    {
        if (infos.Count < 1)
        {
            Debug.LogError($"AudioManager::GetRandomAudioClipInfo - infos is empty");
            return default;
        }
    ...
    /// <param name="infos"></param>
    /// <returns></returns>
    public AudioSource PlayRandom2DClip(List<AudioClipInfo> infos)
    {
        var info = GetRandomAudioClipInfo(infos);
        return Play2DSound(info.clip, info.volume);
    }

    public AudioSource Play2DSound(AudioClipInfo info)
    {
        return Play2DSound(info.clip, info.volume * SoundFxVolume, info.pitch, info.loops);
    }
    ...
    /// </summary>
    /// <returns></returns>
    public AudioSource Play2DSound(AudioClip clip, float volume = 1f, float pitch = 1f, bool loops = false)
    {
        SingleShotAudio fx = CreateNewSoundSource();
        fx.Play2DSound(clip, volume * SoundFxVolume, pitch, loops);
        return fx.Source;
    }
    ...
    /// </summary>
    /// <returns></returns>
    public AudioSource PlaySoundAt(AudioClip clip, Vector3 position, float volume = 1f, float pitch = 1f)
    {
        SingleShotAudio fx = CreateNewSoundSource();
        fx.PlaySoundAt(clip, position, volume * SoundFxVolume, pitch);
    ...
    {
        foreach (var clip in m_soundFxSources)
        {
            if (clip != null)
                clip.Stop();
        }
        m_soundFxSources.Clear();
    }
